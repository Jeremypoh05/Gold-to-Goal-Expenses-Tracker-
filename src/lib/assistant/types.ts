...
export interface Proposal {
  ...
  /** ADDED (dedup restore): a similar EXISTING expense (same amount, same day) found
   *  deterministically at propose time — the card warns "you may be double-logging".
   *  The old quick-mic had this client-side; it was lost when the mic moved onto the
   *  shared engine, and the user promptly triple-logged a hotel. Warning only, never
   *  blocks (repeat same-day spends of the same amount are legitimate). */
  duplicate?: {
    date: string; // YYYY-MM-DD of the existing row
    amount: number;
    currency: Currency;
    category: CategoryKey;
    note: string;
